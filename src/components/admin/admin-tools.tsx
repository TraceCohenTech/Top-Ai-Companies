...
export function AdminTools({ companies }: Props) {
  ...
  const handleExport = () => {
    const csv = Papa.unparse(companies.map(c => ({
      ...
      Notes: c.notes || '',
      Website: c.website || '',
      'Funding Total USD': c.funding_total_usd || '',
      'Last Round': c.last_round_name || '',
      'Last Round USD': c.last_round_usd || '',
      'Last Round Date': c.last_round_date || '',
      'Valuation USD': c.valuation_usd || '',
      'Company Type': c.company_type,
      'Ticker': c.ticker || '',
      'Employee Count': c.employee_count || '',
      'Revenue Estimate USD': c.revenue_estimate || '',
      'Investors': c.investor_summary || '',
